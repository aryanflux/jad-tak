...
export async function POST(request: NextRequest) {
  let body: { text?: unknown; categoryCode?: unknown; embedding?: unknown };
  try {
  ...
  try {
    ...
    const embeddingValues: number[] | null =
      suppliedEmbedding &&
      suppliedEmbedding.length === 384 &&
      suppliedEmbedding.every((value) => typeof value === 'number' && Number.isFinite(value))
        ? suppliedEmbedding as number[]
        : null;
    if (embeddingValues) {
